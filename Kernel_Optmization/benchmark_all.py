...
def build_kernel_registry(device: torch.device,
                          mode: str = "perturb") -> OrderedDict:
    """Build registry of all available kernels.

    Args:
        device: CUDA device.
        mode: 'perturb' for fused RNG+perturbation (trainer.py baseline),
              'rng' for pure RNG generation only.
    """
    ...
    # --- PyTorch baseline ---
    if mode == "rng":
        # Pure RNG: in-place fill with N(0,1), no temporary allocation
        def _pytorch(buf, seed):
            torch.manual_seed(seed)
            buf.normal_()
        reg["PyTorch randn"] = (_pytorch,
            "torch.normal_ in-place (pure RNG)")
    else:
        # Perturbation: matches trainer.py zo_perturb_parameters
        # Original code: torch.manual_seed(seed)
        ...
        def _pytorch(buf, seed):
            ...
            buf.add_(z)
        reg["PyTorch randn"] = (_pytorch,
            "torch.normal + add (matches trainer.py zo_perturb_parameters)")
...
def benchmark_kernel(
    ...
    if mode == "rng":
        # Pure RNG: write-only = 1 store per element
        xfer_bytes = mem_bytes
    else:
        # Fused perturb: load + store = 2 * n * 4 bytes
        xfer_bytes = 2 * mem_bytes
    bw_gbs = (xfer_bytes / 1e9) / (med_us / 1e6) if med_us > 0 else 0.0
...
def export_csv(
    ...
    fname = f"benchmark_{mode}_{timestamp}.csv"
...
def generate_latex(
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    fname = f"benchmark_{mode}_{timestamp}.tex"
    fpath = os.path.join(output_dir, fname)
    ...
    L(r"\centering")
    if mode == "rng":
        L(r"\caption{Pure RNG generation time (median, $\mu$s) and speedup vs "
          r"\texttt{torch.normal\_} across OPT model layer dimensions on "
          + _latex_escape(gpu_name) + r".}")
        L(r"\label{tab:rng_benchmark}")
    else:
        L(r"\caption{Fused perturbation time (median, $\mu$s) and speedup vs "
          r"\texttt{torch.normal} + add across OPT model layer dimensions on "
          + _latex_escape(gpu_name) + r".}")
        L(r"\label{tab:perturb_benchmark}")
    if n_dims > 5:
        L(r"\small")
    if n_dims > 8:
        L(r"\scriptsize")
    ...
    if sorted_dims:
        ...
        if largest_label in results:
            ...
            if mode == "rng":
                bw_type = "Write-only"
                L(r"\caption{Effective write-only bandwidth (GB/s) at "
                  + _latex_escape(largest_label)
                  + r" ($n=" + f"{largest_n:,}" + r"$), pure RNG mode.}")
                L(r"\label{tab:rng_bandwidth}")
            else:
                bw_type = "Read+write"
                L(r"\caption{Effective read+write bandwidth (GB/s) at "
                  + _latex_escape(largest_label)
                  + r" ($n=" + f"{largest_n:,}" + r"$), fused perturbation mode.}")
                L(r"\label{tab:perturb_bandwidth}")
...
def main():
    ...
    parser.add_argument("--mode", choices=["perturb", "rng"],
                        default="perturb",
                        help="'perturb': fused RNG+perturbation matching "
                             "trainer.py (default); "
                             "'rng': pure RNG generation only")
    ...
    mode_desc = ("pure RNG generation" if args.mode == "rng"
                 else "fused RNG + perturbation")
    ...
    registry = build_kernel_registry(device, mode=args.mode)
    ...
    for dim_label, n_elements in sorted_dims:
        ...
        for name, (fn, _) in registry.items():
            stats = benchmark_kernel(
                ...
                args.warmup, args.repeat, mode=args.mode,
            )
            if "error" in stats:
    ...
    if sorted_dims and sorted_dims[-1][0] in results:
        ...
        bw_label = ("Write-only" if args.mode == "rng"
                    else "Read+Write")
        print(f"\nEffective {bw_label} Bandwidth at {largest}:")
